...
void chip8::emulatecycle(){
  ...
  switch(opcode & 0xF000){
    ...
    case 0x0000:  // 0xxx
      switch(opcode){
        ...
        case 0x00EE:  // Return from subroutine
          ...
          else stackPointer--; programCounter = stack[stackPointer];
          break;
      } break;
    ...
    case 0x9000:  // 9xy0: if V[x] != V[y], skip the next instruction
      if(V[(opcode & 0x0F00) / 0x0100] != V[(opcode & 0x00F0) / 0x0010]) programCounter+=2;
      programCounter++;
      break;
    ...
    case 0xE000:  // Exxx
      switch(opcode & 0x00FF){
        case 0x009E:  // Ex9E: Skip the next instruction if the key indicated by V[x] is pressed
          if(kb.isPressed(V[(opcode & 0x0F00) / 0x0100])) programCounter+=2;
          else programCounter++;
          break;
    ...
    case 0xF000:  // Fxxx
      switch(opcode & 0x00FF){
        ...
        case 0x000A:  // Fx0A: Waits for a keypress, pausing all other execution of instructions, and stores the key value in V[x]
          char key = kb.readKey();
          if(key != 255) V[(opcode & 0x0F00) / 0x0100] = key; programCounter++;
          break;
